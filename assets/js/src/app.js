...
var ColorLabel = React.createClass({
  ...
  render: function() {
    return <span className="color-label" data-toggle="tooltip" data-clipboard-text={this.props.color}>{this.props.color}</span>
  }
});
...
var ColorBox = React.createClass({
  ...
  render: function() {
    ...
    return <div className="color" style={style} onClick={this.handleClick}>
      <h1>Click anywhere or press space to generate random color</h1>
    </div>
  }
});
...
var ColorGnrtr = React.createClass({
  ...
  render: function() {
    return <div>
      <ColorBox onColorChange={this.handleColorChange} color={this.state.color} />
      <ColorLabel color={this.state.color} />
    </div>
  }
});

ReactDOM.render(<ColorGnrtr />, document.getElementById('react_this'));